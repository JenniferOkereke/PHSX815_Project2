# -*- coding: utf-8 -*-
"""
Created on Mon Mar 13 13:39:07 2023

@author: c198o114
"""

# Python code for two dimensional random walk. 
import sys
import numpy
...
for j in range(1,Nwalk+1):
    for i in range(1, Nstep):
        val = random.Categorical(0.15,0.15,0.15,0.15,0.15,0.15) 
        if val == 1: 
            x[i] = x[i - 1] + 1
            y[i] = y[i - 1] 
        elif val == 2: 
...
plt.grid(axis='x', alpha=0.65)
plt.grid(axis='y', alpha=0.65)
plt.xlabel('x')
plt.ylabel('y')
plt.title('Final positions after ' + str(Nwalk) + ' random walks')
plt.show()
f1 = numpy.linspace(0.0,45.0,2000)
f2 = 2*f1/Nstep * numpy.exp(-f1*f1/Nstep)
plt.plot(f1,f2, color='black', linestyle='dashed', label = 'Rayleigh distribution, N= ' + str(Nstep))
n, bins, patches = plt.hist(finald, 15 , density = True, color ='purple', alpha=0.65, fill = True, hatch='/',histtype='step', linewidth=1, label = 'data')
plt.legend(loc='upper right')
plt.title('Different between Data and Rayleigh distributions ')
plt.xlabel('r')
...
n, bins, patches = plt.hist(finaldbias, 15 , density = True, color ='grey', alpha=0.65, fill = True, hatch='/',histtype='step', linewidth=1, label = 'data')
plt.legend(loc='upper right')
plt.title('Different betwween Data and Rayleigh distributions ')
plt.xlabel('r')
plt.ylabel('Probability')
plt.grid(axis='y', alpha=0.65)
plt.show()